...
def mongodog_sniffer(custom=None, callback_before=None, callback_after=None):
    ...
    - `callback_before`: a function that accepts `custom` as the first positional argument followed by the rest of
    positional and keyword arguments used to call the decorated function. If this callback raises `SkipCall` exception
    the decorated function and `callback_after` will not get called and None will be returned.
    - `callback_after`: a function that accepts result of the original function and `custom` as the first two
    positional arguments followed by the rest of the positional and keyword arguments passed to the call.
    """
    ...
    def actual_decorator(func):
        ...
        def replacement(*args, **kwargs):
            """Calls `callback_before`, then calls `func` and finally calls `callback_after`.
...
SNIFFER_CONFIG = [
    ...
    ('collection_find_and_modify', pymongo.collection.Collection, 'find_and_modify'),
    ('collection_find_one', pymongo.collection.Collection, 'find_one'),
    ('collection_group', pymongo.collection.Collection, 'group'),
    ('collection_inline_map_reduce', pymongo.collection.Collection, 'inline_map_reduce'),
...
class Sniffer(object):
    ...
    def __init__(self, reporter, with_traceback=True):
        ...
        for func, cls, mn in self.config:
            custom = {'f': func}
            callback_before = getattr(self, 'callback_before_%s' % func, self.callback_before_generic)
            callback_after = getattr(self, 'callback_after_%s' % func, None)
            original_function = getattr(cls, mn)
            original_function_path = '%s.%s.%s' % (cls.__module__, cls.__name__, mn)
            decorator = mongodog_sniffer(custom, callback_before, callback_after)

            self.original[original_function_path] = original_function
            self.decorated[original_function_path] = decorator(original_function)

    def start(self):
        """Starts sniffing"""
        for key, cls, mn in self.config:
            original_function_path = '%s.%s.%s' % (cls.__module__, cls.__name__, mn)
            setattr(cls, mn, self.decorated[original_function_path])

    def stop(self):
        """Stops sniffing"""
        for key, cls, mn in self.config:
            original_function_path = '%s.%s.%s' % (cls.__module__, cls.__name__, mn)
            setattr(cls, mn, self.original[original_function_path])
    ...
    def report_command(self, command):
        ...
        command_copy = copy.deepcopy(command)  # pymongo tends to modify some things within calls
        self.reporter.report_mongo_command(command_copy, tb)
    ...
    def callback_before_database_command(self, custom, database, command, value=1, check=True, allowable_errors=[],
    ...
    def callback_before_collection_aggregate(self, custom, collection, pipeline):
    ...
    def callback_before_collection_find(self, custom, collection, spec=None, fields=None, skip=0, limit=0,
    ...
    def callback_before_collection_find_and_modify(self, custom, collection, query={}, update=None,
    ...
    def callback_before_collection_find_one(self, custom, collection, spec_or_id=None, fields=None, skip=0, limit=0,
                                            timeout=True, snapshot=False, tailable=False, sort=None, max_scan=None,
                                            as_class=None, slave_okay=False, await_data=False, partial=False,
                                            manipulate=True, read_preference=ReadPreference.PRIMARY, tag_sets=[{}],
    ...
    def callback_before_collection_group(self, custom, collection, key, condition, initial, reduce, finalize=None):
    ...
    def callback_before_collection_inline_map_reduce(self, custom, collection, map, reduce, full_response=False,
                                                     **kwargs):
    ...
    def callback_before_collection_insert(self, custom, collection, doc_or_docs, manipulate=True,
    ...
    def callback_before_collection_map_reduce(self, custom, collection, map, reduce, out, full_response=False,
                                              **kwargs):
    ...
    def callback_before_collection_remove(self, custom, collection, spec_or_id=None, safe=None, **kwargs):
    ...
    def callback_before_collection_save(self, custom, collection, to_save, manipulate=True, safe=None,
    ...
    def callback_before_collection_update(self, custom, collection, spec, document, upsert=False,
